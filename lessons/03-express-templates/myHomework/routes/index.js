...
//might as well use the object oriented syntax
...
router.get('/cats/new', function(req, res, next) {
	...
	colors.push(possibleColors[Math.floor(Math.random()*possibleColors.length)]);
	//fun fact you could just use:
	//	db.add({name, age, colors})
	//if you don't put in a key it auto makes one with the same name as the variable you put in. snazzy af (thanks to the new es6 synax that node supports). Be careful using this on the client though cause legacy browsers won't support it
